...
# Run the application
./bin/$application $parameters --NameOfOutputSummaryFile=$testDir/$outputSummary >$outputDest || err=1
...
then
    cd $testDir
    tar czf $1.log.tar.gz $1.log.out $outputSummary
    rm $1.log.out $outputSummary
    cd -
fi
